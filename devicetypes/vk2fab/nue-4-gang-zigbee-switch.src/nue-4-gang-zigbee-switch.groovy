...
metadata {
definition (name: "Nue 4 Gang Zigbee Switch", namespace: "VK2FAB", author: "VK2FAB") {
    capability "Actuator"
...
tiles(scale: 2) {
    ...
    multiAttributeTile(name:"switch4", type: "device.switch", width: 6, height: 4, canChangeIcon: true){
        tileAttribute ("device.switch4", key: "PRIMARY_CONTROL") { 
            attributeState "on", label:'switch4', action:"off4", icon:"st.switches.light.on", backgroundColor:"#00a0dc", nextState:"turningOff"
            attributeState "off", label:'switch4', action:"on4", icon:"st.switches.light.off", backgroundColor:"#ffffff", nextState:"turningOn"
            attributeState "turningOn", label:'switch4', action:"off4", icon:"st.switches.light.on", backgroundColor:"#00a0dc", nextState:"turningOff"
            attributeState "turningOff", label:'switch4', action:"on4", icon:"st.switches.light.off", backgroundColor:"#ffffff", nextState:"turningOn"
        }
        tileAttribute("device.lastCheckin", key: "SECONDARY_CONTROL") {
            attributeState("default", label:'Last Update: ${currentValue}',icon: "st.Health & Wellness.health9")
        }
...
def parse(String description) {
	...
	if (description?.startsWith('catchall:')) {
		...
		def refreshCmds = zigbee.readAttribute(0x0006, 0x0000, [destEndpoint: 0x10]) +
			...
			zigbee.readAttribute(0x0006, 0x0000, [destEndpoint: 0x13])
...
private Map parseCatchAllMessage(String description) {
	...
	else if (cluster.sourceEndpoint == 0x13)
    {
      log.debug "Switch Four:"
      def onoff = cluster.data[-1]
      if (onoff == 1)
...
def off4() {
	...
	"st cmd 0x${device.deviceNetworkId} 0x13 0x0006 0x0 {}"
}
def on4() {
	log.debug "on4()"
	sendEvent(name: "switch4", value: "on")
	"st cmd 0x${device.deviceNetworkId} 0x13 0x0006 0x1 {}"
}
...
def refresh() {
	log.debug "refreshing"
    	...
    	"st rattr 0x${device.deviceNetworkId} 0x13 0x0006 0x0", "delay 1000",
		]
}
